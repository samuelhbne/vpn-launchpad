...
docker run --name $CTNNAME -p $SOCKSPORT:1080 -p $DNSPORT:53/udp -p $HTTPPORT:8123 -d $IMGNAME:$TARGET -s ${SSHOST} -p ${SSPORT} -b ${LISTENADDR} -l 1080 -k "${SSPASS}" -m "${SSMTHD}" >/dev/null
echo "Done."
echo
